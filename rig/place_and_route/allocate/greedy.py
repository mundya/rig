...
def allocate(vertices_resources,nets,machine,constraints,placements):
 'Allocate resources to vertices on cores arbitrarily using a simple greedy\n    algorithm.\n    ';allocation={};globally_reserved=defaultdict(list);locally_reserved=defaultdict(lambda:defaultdict(list));alignments=defaultdict(lambda:1)
 for constraint in constraints:
  if isinstance(constraint,ReserveResourceConstraint):
   if constraint.location is None:globally_reserved[constraint.resource].append(constraint.reservation)
   else:locally_reserved[constraint.location][constraint.resource].append(constraint.reservation)
  elif isinstance(constraint,AlignResourceConstraint):alignments[constraint.resource]=constraint.alignment
 chip_contents=defaultdict(list)
 for vertex,xy in iteritems(placements):chip_contents[xy].append(vertex)
 for xy,chip_vertices in iteritems(chip_contents):
  resource_pointers={resource:0 for resource in machine.chip_resources}
  for vertex in chip_vertices:
   vertex_allocation={}
   for resource,requirement in iteritems(vertices_resources[vertex]):
    proposed_allocation=None;proposal_overlaps=True
    while proposal_overlaps:
     start=align(resource_pointers[resource],alignments[resource]);proposed_allocation=slice(start,start+requirement);proposal_overlaps=False
     if proposed_allocation.stop>machine[xy][resource]:raise InsufficientResourceError('{} over-allocated on chip {}'.format(resource,xy))
     for reservation in globally_reserved[resource]:
      if slices_overlap(proposed_allocation,reservation):resource_pointers[resource]=reservation.stop;proposal_overlaps=True
     local_reservations=locally_reserved.get(xy,{}).get(resource,[])
     for reservation in local_reservations:
      if slices_overlap(proposed_allocation,reservation):resource_pointers[resource]=reservation.stop;proposal_overlaps=True
    vertex_allocation[resource]=proposed_allocation;resource_pointers[resource]=proposed_allocation.stop
